...
public class PolicyDataDaoImpl implements PolicyDataDao {
	...
	public List<DuePolicies> getDuePolicies(DashboardPara para) throws Exception {
		...
		if (para.getUsertype().equalsIgnoreCase("IC") || para.getUsertype().equalsIgnoreCase("UNL")) {
            ...
            + "inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod "
            + "inner join smtrxnstatus s on p.sbucod=s.sbucod and p.pprsta=s.statid "
            + "where b.sbucod=? and a.loccod IN (?) and p.pprsta in ('PLISU','PLAPS')  "
            + "and concat(b.txnyer,'-',if(length(b.txnmth)='1',concat('0',b.txnmth),b.txnmth),'-01') < curdate() "
            + "group by b.pprnum,b.txnyer,b.txnmth having sum(b.amount) <> 0) x group by x.pprnum order by CAST(x.polnum AS SIGNED) ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new DuePoliciesRowMapper());
			...
			return jdbcTemplate.query("select x.loccod,x.pprnum,x.polnum,x.stadsc,x.ppdini,x.ppdmob,x.amount premium,count(x.pprnum) nofarias,sum(x.amount) totarias from  "
		            ...
		            + "inner join smtrxnstatus s on p.sbucod=s.sbucod and p.pprsta=s.statid "
		            + "where b.sbucod=? and l.rgncod IN (?) and p.pprsta in ('PLISU','PLAPS')  "
		            + "and concat(b.txnyer,'-',if(length(b.txnmth)='1',concat('0',b.txnmth),b.txnmth),'-01') < curdate() "
		            + "group by b.pprnum,b.txnyer,b.txnmth having sum(b.amount) <> 0) x group by x.pprnum order by CAST(x.polnum AS SIGNED) ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new DuePoliciesRowMapper());
			...
			return jdbcTemplate.query("select x.loccod,x.pprnum,x.polnum,x.stadsc,x.ppdini,x.ppdmob,x.amount premium,count(x.pprnum) nofarias,sum(x.amount) totarias from  "
		            ...
		            + "inner join smtrxnstatus s on p.sbucod=s.sbucod and p.pprsta=s.statid "
		            + "where b.sbucod=? and r.zoncod IN (?) and p.pprsta in ('PLISU','PLAPS')  "
		            + "and concat(b.txnyer,'-',if(length(b.txnmth)='1',concat('0',b.txnmth),b.txnmth),'-01') < curdate() "
		            + "group by b.pprnum,b.txnyer,b.txnmth having sum(b.amount) <> 0) x group by x.pprnum order by CAST(x.polnum AS SIGNED) ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new DuePoliciesRowMapper());
	...
	public Integer getCurrentMonthNOP(DashboardPara para) throws Exception {
		...
		if (para.getUsertype().equalsIgnoreCase("IC") || para.getUsertype().equalsIgnoreCase("UNL")) {
			...
			return jdbcTemplate.queryForObject("select count(p.polnum) nop from inproposals p inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod "
		            + "where p.sbucod=? and p.pprsta <> 'INAC' and p.poldat between DATE_FORMAT(NOW() ,'%Y-%m-01') and now() and a.loccod IN (?) ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())},Integer.class);
			
		} else if (para.getUsertype().equalsIgnoreCase("REGION")) {
			
			return jdbcTemplate.queryForObject("select count(p.polnum) nop from inproposals p inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod "
					+ "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code "
		            + "where p.sbucod=? and p.pprsta <> 'INAC' and p.poldat between DATE_FORMAT(NOW() ,'%Y-%m-01') and now() and l.rgncod IN (?) ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())},Integer.class);
			...
			return jdbcTemplate.queryForObject("select count(p.polnum) nop from inproposals p inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod "
					+ "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code "
					+ "inner join inregion r on l.sbu_code=r.sbucod and l.rgncod=r.rgncod "
		            + "where p.sbucod=? and p.pprsta <> 'INAC' and p.poldat between DATE_FORMAT(NOW() ,'%Y-%m-01') and now() and r.zoncod IN (?) ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())},Integer.class);
	...
	public List<PendingPolicies> getPendingPolicies(DashboardPara para) throws Exception {
		...
		if (para.getUsertype().equalsIgnoreCase("IC") || para.getUsertype().equalsIgnoreCase("UNL")) {
			...
			return jdbcTemplate.query("select a.loccod,p.pprnum,a.agncod,p.ppdini,p.totprm,group_concat(m.addnot) requirment from inproposals p  "
		            + "inner join inpropmedicalreq m on p.sbucod=m.sbucod and p.loccod=m.loccod and p.pprnum=m.pprnum and p.prpseq=m.prpseq "
		            + "inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod "
		            + "where p.sbucod=? and p.pprsta <> 'INAC' and p.poldat is null and a.loccod IN (?) "
		            + "group by p.pprnum order by CAST(p.pprnum AS SIGNED) DESC ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new PendingPoliciesRowMapper());
			...
			return jdbcTemplate.query("select a.loccod,p.pprnum,a.agncod,p.ppdini,p.totprm,group_concat(m.addnot) requirment from inproposals p  "
		            ...
		            + "inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod "
		            + "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code "
		            + "where p.sbucod=? and p.pprsta <> 'INAC' and p.poldat is null and l.rgncod IN (?) "
		            + "group by p.pprnum order by CAST(p.pprnum AS SIGNED) DESC ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new PendingPoliciesRowMapper());
			...
			return jdbcTemplate.query("select a.loccod,p.pprnum,a.agncod,p.ppdini,p.totprm,group_concat(m.addnot) requirment from inproposals p  "
		            ...
		            + "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code "
		            + "inner join inregion r on l.sbu_code=r.sbucod and l.rgncod=r.rgncod "
		            + "where p.sbucod=? and p.pprsta <> 'INAC' and p.poldat is null and r.zoncod IN (?) "
		            + "group by p.pprnum order by CAST(p.pprnum AS SIGNED) DESC ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new PendingPoliciesRowMapper());
	...
	public PolicySummary getPolicySummary(DashboardPara para) throws Exception {
		...
		if (para.getUsertype().equalsIgnoreCase("BRANCH")) {
            ...
            + "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code  "
            + "where p.sbucod=? and l.loc_code IN (?) and p.pprsta in ('PLISU','PLAPS','PLAPP') ) x ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new PolicySummaryRowMapper());
			...
			return jdbcTemplate.queryForObject("select sum(x.plisucount) plisucount, sum(x.plisuamount) plisuamount, "
		            ...
		            + "from inproposals p inner join inagentmast a on p.sbucod=a.sbucod and p.advcod=a.agncod  "
		            + "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code  "
		            + "where p.sbucod=? and l.rgncod IN (?) and p.pprsta in ('PLISU','PLAPS','PLAPP') ) x ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new PolicySummaryRowMapper());
			...
			return jdbcTemplate.queryForObject("select sum(x.plisucount) plisucount, sum(x.plisuamount) plisuamount, "
		            ...
		            + "inner join rms_locations l on a.sbucod=l.sbu_code and a.loccod=l.loc_code  "
		            + "inner join inregion r on l.sbu_code=r.sbucod and l.rgncod=r.rgncod "
		            + "where p.sbucod=? and r.zoncod IN (?) and p.pprsta in ('PLISU','PLAPS','PLAPP') ) x ", new Object[] { "450",calculationUtils.getPara(para.getDashpara())}, new PolicySummaryRowMapper());
